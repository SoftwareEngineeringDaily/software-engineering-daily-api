...
router
  ...
  .get(expressJwt({ secret: config.jwtSecret }), loadFullUser, postCtrl.recommendations);
...
router
  .route('/:postId/related-links')
  .get(expressJwt({ secret: config.jwtSecret, credentialsRequired: false }), relatedLinkCtrl.list);

// Add a related-link to postId:
router
  .route('/:postId/related-link')
  .post(
    expressJwt({ secret: config.jwtSecret }),
