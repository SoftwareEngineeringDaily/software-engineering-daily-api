...
import convertPostsToAdFreeRssFeed from '../helpers/post.helper';
...
function get(req, res, next) {
  ...
  return res.json(convertPostsToAdFreeRssFeed(req.post.toObject(), req.fullUser, next));
}
...
function list(req, res, next) {
  ...
  Post.list(query)
    .then(posts => res.json(convertPostsToAdFreeRssFeed(posts, req.fullUser, next)))
    .catch(e => next(e));
}
...
function recommendations(req, res, next) {
  ...
  raccoon
    ...
    .then(posts =>
      //eslint-disable-line
      res.json(convertPostsToAdFreeRssFeed(posts, req.fullUser, next)))
    .catch((e) => {
      next(e);
    });
}
